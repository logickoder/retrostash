...
Hosts the Compose Multiplatform `App()` from `:app` on iOS for local development. Not consumed by
...
Add Input File: `$(SRCROOT)/../app/build.gradle.kts` (any sentinel; forces Xcode to re-run the
...
| Setting                  | Value                                                                   |
|--------------------------|-------------------------------------------------------------------------|
| `Framework Search Paths` | `$(SRCROOT)/../app/build/xcode-frameworks/$(CONFIGURATION)/$(SDK_NAME)` |
...
- **`embedAndSignAppleFrameworkForXcode` task not found** —
  verify [app/build.gradle.kts](../app/build.gradle.kts) declares iOS targets with
